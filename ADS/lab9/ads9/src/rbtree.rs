...
///  ```
pub struct RBTree<K: Ord + Copy, V> {
    root: NodePtr<K, V>,
    len: usize,
}

// Drop all owned pointers if the tree is dropped
impl<K: Ord + Copy, V> Drop for RBTree<K, V> {
    #[inline]
    fn drop(&mut self) {
        self.clear();
    }
...
impl<K: Ord + Copy, V: Clone> Clone for RBTree<K, V> {
...
impl<K: Ord + Debug + Copy, V: Debug> RBTree<K, V> {
...
impl<K: Ord + Copy, V> FromIterator<(K, V)> for RBTree<K, V> {
...
/// RBTree into iter
impl<K: Ord + Copy, V> Extend<(K, V)> for RBTree<K, V> {
...
impl<K: Ord + Copy, V> IntoIterator for RBTree<K, V> {
    type Item = (K, V);
...
impl<K: Ord + Copy, V> RBTree<K, V> {
    ...
    pub fn parent(&self, key: K) -> Option<K> {
        let node = self.find_node(&key).parent();
        if node.is_null() {
            return None 
        }
        unsafe { Some((*node.0).key) }
    }

    pub fn left(&self, key: K) -> Option<K> {
        let node = self.find_node(&key).left();
        if node.is_null() {
            return None 
        }
        unsafe { Some((*node.0).key) }
    ...
    pub fn right(&self, key: K) -> Option<K> {
        let node = self.find_node(&key).right();
        if node.is_null() {
            return None 
        }
        unsafe { Some((*node.0).key) }
    ...
    pub fn color(&self, key: K) -> Color {
        self.find_node(&key).get_color()
    }
