...
export default {
  ...
  mounted() {
    this.$emit('help', 'Щоб побудувати паралелограм виберіть три точки на координатній площині, після цього зявиться можливість змінити параметри афінних перетворень в меню зліва, запустити або зупинити анімацію перетворень кнопкою під координтною площиною та зберегти зображення. Також доступна можливість зміни величини одиничного відрізку шляхом користування колесиком мишки при наведенні на координатну площину');
    this.$nextTick(() => {
      const chart = this.$refs.chart.chart;
      chart.zoom(-1000);
      chart.update();
