...
int main() {
    ...
    STARTUPINFO* si = new STARTUPINFO[2];
    PROCESS_INFORMATION* pi = new PROCESS_INFORMATION[2];
    SECURITY_ATTRIBUTES sap;
    SECURITY_ATTRIBUTES sath;
    HANDLE* handles = new HANDLE[2];
    //si.lpTitle = (LPWSTR)(L"MyNotepad").c_str();
    ...
    std::wstring comandline =
        L"";
    std::wstring arguments[] = { L"15 15", L"20 20" };
    std::wstring finalComandLine;
    ...
    for (size_t i = 0; i < 2; i++) {
        ...
        finalComandLine = comandline + arguments[i];
        if (CreateProcess(NULL, (LPWSTR)finalComandLine.c_str(), NULL, NULL, NULL,
            CREATE_NEW_CONSOLE | BELOW_NORMAL_PRIORITY_CLASS | CREATE_SUSPENDED, NULL, NULL, &si[i], &pi[i])) {
            handles[i] = pi[i].hProcess;
            ResumeThread(pi[i].hThread);
            /*WaitForSingleObject(pi[i].hProcess, INFINITE);             //_getch();
            CloseHandle(pi[i].hProcess);
            CloseHandle(pi[i].hThread);
        }
        else
            std::cout << "process failed to create\n";
    }
